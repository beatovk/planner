...
from packages.wp_models.place import Place as PlaceModel
from packages.wp_core.scorer import coolness
from dateutil import parser as dtp
from packages.wp_core.utils.dates import normalize_bkk_day
from packages.wp_core.utils.hash import generate_etag
from packages.wp_core.metrics import metrics
from tools.questionnaire import load_categories, selected_ids_to_tags

# Event-related imports protected by feature flag
if not events_disabled():
    from packages.wp_events.service import plan_week, format_week_plan, build_daily_cards
    from packages.wp_events.live_events import load_source_map, fetch_for_categories
import time
...
async def lifespan(app):
    """Инициализация Redis на старте приложения."""
    try:
        from packages.wp_cache.cache import ensure_client
        app.state.redis = ensure_client()  # прогреваем соединение
        print("Redis client initialized successfully")
    except Exception as e:
        print(f"Failed to initialize Redis: {e}")
        app.state.redis = None
    yield
    # redis-py sync: явного close не нужно

app = FastAPI(title="Week Planner MVP", lifespan=lifespan)
...
def query_analyzer():
    """Страница для тестирования Query Analyzer Agent"""
...
async def api_analyze_query(request: Dict[str, Any]):
    """API endpoint для анализа запросов через Query Analyzer Agent"""
    try:
        from packages.wp_agents.agents.query_analyzer_agent import QueryAnalyzerAgent
        from packages.wp_agents.base import AgentContext
...
# TEMPORARILY DISABLED - HAS SYNTAX ISSUES
if False:
    pass  # Temporary placeholder
        
# DISABLED FUNCTION CONTENT - HAS SYNTAX ISSUES
"""
...
"""
# END OF DISABLED FUNCTION CONTENT

# Events endpoints when events are disabled
if events_disabled():
    # Events endpoints when events are disabled
    @app.post("/api/events")
    @app.post("/api/events/")
...
async def api_places_warm_cache(city: str = "bangkok", flags: str = ""):
    ...
    try:
        ...
        results = service.warm_cache(city, flag_list)
...
def api_places_search(q: str = "", city: str = "bangkok", limit: int = 20):
    """
    Search places by query string.
    
    Args:
        q: Search query (e.g., "food", "rooftops", "parks")
        city: City name (default: bangkok)
        limit: Maximum number of places to return
    """
    try:
        ...
        # Map common queries to flags
        query_to_flags = {
            "food": ["food_dining", "restaurants", "cafes"],
            "rooftops": ["rooftop", "bars", "restaurants"],
            "parks": ["parks", "outdoor", "nature"],
            "bars": ["bars", "nightlife", "drinks"],
            "restaurants": ["food_dining", "restaurants"],
            "cafes": ["food_dining", "cafes"],
            "shopping": ["shopping", "retail"],
            "culture": ["art_exhibits", "museums", "theaters"],
            "outdoor": ["parks", "outdoor", "nature"],
            "nightlife": ["bars", "nightlife", "entertainment"]
        }
        
        # Find matching flags for the query
        matching_flags = []
        query_lower = q.lower().strip()
        
        for key, flags in query_to_flags.items():
            if key in query_lower or any(flag in query_lower for flag in flags):
                matching_flags.extend(flags)
        
        # If no specific flags found, search in all places
        if not matching_flags:
            places, _, _ = service.get_all_places(city, limit)
        else:
            # Remove duplicates and get unique flags
            unique_flags = list(set(matching_flags))
            places, _, _ = service.get_places_by_flags(city, unique_flags, limit)
        ...
        for place in places:
            ...
            if hasattr(place, 'category') and place.category:
                ...
                if 'food' in place.category.lower() or 'restaurant' in place.category.lower():
                    place_dict['image_url'] = 'https://images.unsplash.com/photo-1517248135467-4c7edcad34c4?w=400&h=300&fit=crop'
                elif 'bar' in place.category.lower():
                    place_dict['image_url'] = 'https://images.unsplash.com/photo-1514933651103-005eec06c04b?w=400&h=300&fit=crop'
                elif 'park' in place.category.lower() or 'outdoor' in place.category.lower():
                    place_dict['image_url'] = 'https://images.unsplash.com/photo-1441974231531-c6227db76b6e?w=400&h=300&fit=crop'
                else:
                    place_dict['image_url'] = 'https://images.unsplash.com/photo-1480714378408-67cf0d13bcff?w=400&h=300&fit=crop'
            else:
                # Default image
                place_dict['image_url'] = 'https://images.unsplash.com/photo-1480714378408-67cf0d13bcff?w=400&h=300&fit=crop'
...
@app.get("/api/places/universal/search")
def api_places_universal_search(q: str = "", city: str = "bangkok", limit: int = 20):
    """
    Search places from our universal database (with geo validation data).
    ...
    try:
        ...
        # Connect to our universal database
        db_path = Path(__file__).parent.parent.parent / "data" / "wp_universal.db"
        
        if not db_path.exists():
            return {
                "success": False,
                "error": "Universal database not found",
                "places": [],
                "total": 0
            }
        ...
        query_to_categories = {
            ...
            "rooftops": ["rooftop", "bar", "restaurant"],
            "parks": ["park", "outdoor", "nature"],
            "bars": ["bar", "nightlife", "pub"],
            "restaurants": ["restaurant", "food"],
            "cafes": ["cafe", "coffee"],
            "shopping": ["shopping", "retail", "market"],
            "culture": ["museum", "gallery", "theater"],
        ...
        for key, categories in query_to_categories.items():
            if key in query_lower or any(cat in query_lower for cat in categories):
        ...
        if matching_categories:
            ...
            sql = f"""
                SELECT id, name, description, description_clean, address, 
                       geo_lat, geo_lng, scraped_rating, scraped_reviews_count,
                       scraped_price_level, price, phone, opening_hours,
                       category, google_maps_url, features
                FROM places 
                WHERE category IN ({placeholders})
                ORDER BY scraped_reviews_count DESC, name
                LIMIT ?
            """
            params = matching_categories + [limit]
        else:
            # Search all places
            sql = """
                SELECT id, name, description, description_clean, address, 
        ...
        # Convert to places data
        places_data = []
        for row in rows:
            (place_id, name, description, description_clean, address, 
             geo_lat, geo_lng, scraped_rating, scraped_reviews_count,
             scraped_price_level, price, phone, opening_hours,
             category, google_maps_url, features) = row
            
            # Use clean description if available, otherwise original
            display_description = description_clean if description_clean else description
            ...
            if category:
                ...
                elif 'park' in category.lower() or 'outdoor' in category.lower():
                    image_url = 'https://images.unsplash.com/photo-1441974231531-c6227db76b6e?w=400&h=300&fit=crop'
                else:
                    image_url = 'https://images.unsplash.com/photo-1480714378408-67cf0d13bcff?w=400&h=300&fit=crop'
            else:
                image_url = 'https://images.unsplash.com/photo-1480714378408-67cf0d13bcff?w=400&h=300&fit=crop'
            ...
            place_data = {
                ...
                "scraped_price_level": scraped_price_level,
                "price": price,
                "phone": phone,
        ...
        return {
            ...
            "places": places_data,
            "total": len(places_data),
            "categories_used": matching_categories,
            "source": "universal_database"
        }
        
    except Exception as e:
        log.error(f"Error searching universal places: {e}")
        return {
            "success": False,
            "error": f"Failed to search universal places: {str(e)}",
            "places": [],
            "total": 0
        }
