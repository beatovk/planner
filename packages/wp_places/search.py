...
def ensure_fts(engine: Engine) -> None:
    ...
    # Читаем SQL миграцию
    migration_file = (
        Path(__file__).parent.parent.parent / "search" / "fts5_migrations.sql"
    )
...
def search_by_category(
    engine: Engine, query: str, category: str, limit: int = 20
) -> List[Dict[str, Any]]:
    """
    ...
    with engine.connect() as conn:
        ...
        result = conn.execute(
            sql, {"query": query, "category": category_pattern, "limit": limit}
        )
