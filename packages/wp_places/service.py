...
class PlacesService:
    ...
    def _cache_places(
        self, city: str, flag: str, places: List[Place], ttl: int = 3600
    ) -> bool:
    ...
    def _get_cached_places(self, city: str, flag: str) -> Optional[List[Place]]:
        ...
        try:
            ...
            if cached_data is not None:
                ...
                logger.info(
                    f"Retrieved {len(places)} places from cache for {city}:{flag}"
                )
                return places
            else:
    ...
    def _warm_places_cache(
        self, city: str, flags: List[str], ttl: int = 3600
    ) -> Dict[str, int]:
            ...
            try:
                ...
                if places:
                    ...
                    try:
                        ...
                        logger.info(
                            f"Saved {saved_count} places to database for {city}:{flag}"
                        )
                    except Exception as e:
                    ...
                    if self._cache_places(city, flag, places, ttl):
                        ...
                        logger.info(
                            f"Warmed cache for {city}:{flag} with {len(places)} places"
                        )
                    else:
                        results[flag] = 0
    ...
    def _get_cache_stats(self, city: str) -> Dict[str, Any]:
        ...
        if not is_cache_enabled():
            return {"city": city, "cache_enabled": False, "error": "Cache disabled"}
    ...
    def get_places_by_flags(
            ...
            try:
                ...
                for flag in flags:
                    ...
                    if places:
                        ...
                        logger.info(
                            f"Got {len(places)} places from cache for flag {flag}"
                        )
                    else:
                ...
                if cached_places:
                    ...
                    if filtered_places:
                        logger.info(
                            f"Returning {len(filtered_places)} places from cache"
                        )
                        return filtered_places
                    else:
                        logger.info(
                            "No places passed filtering, falling back to database"
                        )
            except Exception as e:
        ...
        try:
            ...
            if places_data:
                logger.info(
                    f"Retrieved {len(places_data)} places from database for {city}:{flags}"
                )
    ...
    def get_all_places(
        self, city: str, limit: Optional[int] = None, use_cache: bool = True
    ) -> List[Place]:
        """
        ...
        try:
            ...
            if places_data:
                logger.info(
                    f"Retrieved {len(places_data)} places from database for {city}"
                )
                # Конвертируем в Place объекты
                places = []
                for place_dict in places_data:
    ...
    def _fetch_and_save_places(
        self, city: str, flags: List[str], limit: Optional[int] = None
    ) -> List[Place]:
        """
                ...
                if places:
                    ...
                    try:
                        if self._cache_places(city, flag, places):
                            logger.info(
                                f"Cached {len(places)} places for {city}:{flag}"
                            )
                        else:
                            logger.warning(f"Failed to cache places for {city}:{flag}")
                    except Exception as e:
    ...
    def _filter_places_by_flags(
        self, places: List[Place], flags: List[str]
    ) -> List[Place]:
    ...
    def warm_cache(
        self, city: str, flags: Optional[List[str]] = None, ttl: int = 3600
    ) -> Dict[str, int]:
        """
    ...
    def refresh_places(
        self, city: str, flags: Optional[List[str]] = None
    ) -> Dict[str, int]:
        """
        ...
        try:
            ...
            if is_cache_enabled() and places:
                ...
                self._cache_client.set_json(
                    cache_key, places_data_for_cache, get_cache_ttl("short")
                )
