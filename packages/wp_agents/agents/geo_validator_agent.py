...
from packages.wp_agents.base import (
    BaseAgent,
    AgentContext,
    AgentResult,
    AgentPriority,
...
class GeoValidatorAgent(BaseAgent):
    ...
    def __init__(
        self, db_path: str = "data/wp.db", google_api_key: Optional[str] = None
    ):
        super().__init__(
            name="geo_validator",
            version="2.2.0",
        ...
        self.bangkok_center = {"lat": 13.7563, "lng": 100.5018}
    ...
    async def _execute_agent(self, context: AgentContext) -> AgentResult:
        ...
        try:
            ...
            if not places:
                return AgentResult(
                    success=False, error="Нет мест для валидации", data={}
                )
            ...
            for i, place in enumerate(places):
                try:
                    self.logger.info(
                        f"Валидация места {i+1}/{len(places)}: {place.get('name', 'Unknown')}"
                    )
                    ...
                    if validated_place:
                        ...
                        errors.append(
                            f"Не удалось валидировать {place.get('name', 'Unknown')}"
                        )
            ...
            return AgentResult(
                ...
                data={
                    ...
                    "maps_urls_created": len(
                        [p for p in validated_places if p.google_maps_url]
                    ),
                    "extended_data_collected": len(
                        [
                            p
                            for p in validated_places
                            if p.scraped_rating or p.scraped_price_level
                        ]
                    ),
                    "errors": errors,
                },
            ...
            return AgentResult(success=False, error=str(e), data={})
    ...
    async def _validate_place(
        ...
        try:
            ...
            if not coordinates:
                self.logger.warning(
                    f"Не удалось получить координаты для {place['name']}"
                )
                return None
            ...
            scraped_data = await self._scrape_google_maps_data(
                place["name"], place["address"]
            )

            # 6. Получение точной ссылки на Google Maps (без API)
            google_maps_url = await self._get_google_maps_place_url(
                place["name"], place["address"]
            )

            # Если точная ссылка не найдена, генерируем поисковую
            if not google_maps_url:
                google_maps_url = self._generate_google_maps_url(
                    place["name"], coordinates, place["address"]
                )
            ...
            self.logger.error(
                f"Ошибка валидации места {place.get('name', 'Unknown')}: {e}"
            )
            return None
    ...
    async def _geocode_with_google_api(
        self, name: str, address: Optional[str]
    ) -> Optional[Dict[str, float]]:
        ...
        try:
            ...
            params = {"address": query, "key": self.google_api_key, "region": "th"}
    ...
    async def _geocode_with_alternative_methods(
        self, name: str, address: Optional[str]
    ) -> Optional[Dict[str, float]]:
        ...
        try:
            ...
            if hasattr(self, "raw_data") and self.raw_data:
                ...
                if raw_data.get("geo_lat") and raw_data.get("geo_lng"):
                    return {"lat": raw_data["geo_lat"], "lng": raw_data["geo_lng"]}
    ...
    def _find_coordinates_by_name_and_district(
        self, name: str, address: Optional[str]
    ) -> Optional[Dict[str, float]]:
        ...
        for district, district_info in self.bangkok_districts.items():
            if (
                district in name_lower
                or district in address_lower
                or any(
                    keyword in name_lower
                    for keyword in district_info.get("keywords", [])
                )
            ):
    ...
    def _validate_city(self, coordinates: Dict[str, float]) -> bool:
        ...
        return (
            bangkok_bounds["min_lat"] <= coordinates["lat"] <= bangkok_bounds["max_lat"]
            and bangkok_bounds["min_lng"]
            <= coordinates["lng"]
            <= bangkok_bounds["max_lng"]
        )
    ...
    def _calculate_distance_from_center(self, coordinates: Dict[str, float]) -> float:
        ...
        return self._calculate_distance(
            coordinates["lat"],
            coordinates["lng"],
            self.bangkok_center["lat"],
            self.bangkok_center["lng"],
        )
    ...
    def _generate_google_maps_url(
        self, name: str, coordinates: Dict[str, float], address: Optional[str] = None
    ) -> str:
    ...
    async def _get_google_maps_place_url(
        ...
        try:
            ...
            params = {"q": search_query, "hl": "en"}  # Английский язык
            ...
            async with self.session.get(
                ...
                place_link_match = re.search(
                    r'href="(/maps/place/[^"]+)"', html_content
                )
                if place_link_match:
            ...
            self.logger.error(
                f"Ошибка получения ссылки на место Google Maps для {name}: {e}"
            )
            return None

    async def _scrape_google_maps_data(
        self, name: str, address: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            async with self.session.get(
                ...
                rating_match = re.search(
                    r"(\d+\.?\d*)\s*stars?", html_content, re.IGNORECASE
                )
                if rating_match:
                ...
                rating_match2 = re.search(
                    r"(\d+\.?\d*)\s*out of\s*5", html_content, re.IGNORECASE
                )
                if rating_match2:
                ...
                reviews_match = re.search(
                    r"(\d+(?:,\d+)*)\s*reviews?", html_content, re.IGNORECASE
                )
                if reviews_match:
                ...
                if price_match:
                    ...
                    scraped_data["price_level"] = self._map_google_price_level(
                        price_level
                    )

                # 4. Часы работы
                hours_match = re.search(
                    r"Open\s*(\d{1,2}:\d{2}\s*(?:AM|PM))\s*-\s*(\d{1,2}:\d{2}\s*(?:AM|PM))",
                    html_content,
                    re.IGNORECASE,
                )
                if hours_match:
                    scraped_data["opening_hours"] = (
                        f"{hours_match.group(1)} - {hours_match.group(2)}"
                    )

                # 5. Телефон
                phone_match = re.search(
                    r"(\+\d{1,3}\s*\d{1,4}\s*\d{1,4}\s*\d{1,4})", html_content
                )
                if phone_match:
                ...
                category_match = re.search(
                    r"(restaurant|cafe|bar|coffee shop|bakery)",
                    html_content,
                    re.IGNORECASE,
                )
                if category_match:
                ...
                # 9. Популярные часы
                popular_hours_match = re.search(
                    r"Popular\s*times\s*([^<]+)", html_content, re.IGNORECASE
                )
                ...
                if not address:
                    address_match = re.search(
                        r"Address[:\s]+([^<]+)", html_content, re.IGNORECASE
                    )
                    if address_match:
                        scraped_data["scraped_address"] = address_match.group(1).strip()

                self.logger.info(
                    f"Собрано данных для {name}: {len(scraped_data)} полей"
                )
                return scraped_data
    ...
    async def _get_google_maps_data(
        self, name: str, coordinates: Dict[str, float]
    ) -> Dict[str, Any]:
            ...
            async with self.session.get(url, params=params) as response:
                ...
                if data.get("status") == "OK" and data.get("results"):
                    ...
                    return {
                        ...
                        "price_level": self._map_google_price_level(
    ...
    def _map_google_price_level(
        self, google_price_level: Optional[int]
    ) -> Optional[str]:
    ...
    async def _save_validated_places(
        self, validated_places: List[GeoValidatedPlace]
    ) -> int:
        ...
        try:
            ...
            try:
                cursor.execute(
                    "ALTER TABLE places ADD COLUMN scraped_reviews_count INTEGER"
                )
            except:
                pass
            ...
            for place in validated_places:
                try:
                    ...
                    saved_count += 1
                    self.logger.info(
                        f"Обновлено место: {place.name} (lat: {place.geo_lat}, lng: {place.geo_lng})"
                    )
    ...
    def can_execute(self, context: AgentContext) -> bool:
        ...
        return self.is_enabled() and context.city and self.status != AgentStatus.RUNNING
