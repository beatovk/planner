...
def cache_client(mock_redis_client):
    ...
    with patch('packages.wp_cache.cache.get_sync_client', return_value=mock_redis_client):
...
class TestCacheClient:
    ...
    def test_init_without_redis(self):
        ...
        with patch('packages.wp_cache.cache.get_sync_client', side_effect=Exception("Redis unavailable")):
            with patch('packages.wp_cache.cache.should_bypass_redis', return_value=False):
                client = CacheClient()
...
class TestCacheIntegration:
    ...
    def test_cache_pattern_clearing(self, cache_client, mock_redis_client):
        ...
        mock_redis_client.keys.return_value = ["v2:places:bangkok:flag:food", "v2:places:bangkok:flag:drinks"]
        
        result = cache_client.clear_pattern("v2:places:bangkok:*")
        
        assert result == 2
        mock_redis_client.keys.assert_called_once_with("v2:places:bangkok:*")
        mock_redis_client.delete.assert_called_once_with("v2:places:bangkok:flag:food", "v2:places:bangkok:flag:drinks")
