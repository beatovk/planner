...
def test_init_schema(temp_db):
    ...
    with temp_db.connect() as conn:
        result = conn.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='places'")
...
def test_save_places(temp_db, sample_places):
    ...
    with temp_db.connect() as conn:
        ...
        result = conn.execute("SELECT name, city, rating FROM places WHERE id = 'place_1'")
        row = result.fetchone()
...
def test_get_places_by_flags(temp_db, sample_places):
    ...
    # Проверяем лимит
    all_places = get_places_by_flags(temp_db, ["food_dining", "parks", "art_exhibits"], limit=2)
...
def test_update_existing_place(temp_db, sample_places):
    ...
    with temp_db.connect() as conn:
        result = conn.execute("SELECT rating, description FROM places WHERE id = 'place_1'")
        row = result.fetchone()
        assert row[0] == 5.0
