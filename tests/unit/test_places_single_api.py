...
class TestPlacesSingleAPI:
    ...
    def test_places_endpoints_exist_in_main_app(self):
        ...
        for endpoint in expected_endpoints:
            assert any(
                route.path == endpoint for route in routes
    ...
    def test_no_duplicate_places_endpoints(self):
        ...
        duplicates = {
            path: count for path, count in endpoint_counts.items() if count > 1
        }
    ...
    def test_places_endpoints_have_correct_methods(self):
        ...
        for endpoint, expected_methods_list in expected_methods.items():
            route = next(
                (r for r in routes if hasattr(r, "path") and r.path == endpoint), None
            )
            ...
            if hasattr(route, "methods"):
                ...
                for method in expected_methods_list:
                    ...
                    ), f"Method {method} not found for {endpoint}"
    ...
    def test_no_secondary_places_api_imported(self):
        ...
        assert (
            "from src.api.places_api import" not in content
        ), "Secondary places API imported in app_factory.py"
        assert (
            "register_places_routes" not in content
        ), "register_places_routes called in app_factory.py"
    ...
    def test_places_endpoints_use_core_service(self):
        ...
        assert (
            "from packages.wp_places.api import places_router" in content
        ), "places_router not imported from packages.wp_places.api"
        assert (
            "app.include_router(places_router)" in content
        ), "places_router not included in app"
