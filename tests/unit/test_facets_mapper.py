...
def test_categories_to_facets_mapping():
    ...
    assert "art_exhibits" in categories, f"'art_exhibits' should be in categories, got {categories}"
...
def test_fallback_flags_never_empty():
    ...
    assert "all" in flags or any(flag in flags for flag in ["art", "culture"]), \
...
def test_multiple_categories_mapping():
    ...
    flags = facets.get("flags", [])
    assert len(flags) > 0, f"flags should not be empty for multiple categories, got {flags}"
    ...
    assert "culture" in flags, f"'culture' should be in flags for culture category, got {flags}"
...
def test_fallback_flags_with_valid_facets():
    ...
    assert len(flags) == len(set(flags)), f"flags should not contain duplicates, got {flags}"
...
def test_edge_cases():
    ...
    assert facets["categories"] == set(), "Empty categories should result in empty categories set"
...
def test_map_event_to_flags_multiple():
    ...
    flags = map_event_to_flags(event)
    assert "art_exhibits" in flags, f"Art workshop should map to art_exhibits, got {flags}"
    ...
    assert len(flags) >= 3, f"Multi-category event should map to multiple flags, got {flags}"
...
def test_map_event_to_flags_missing_fields():
    ...
    flags = map_event_to_flags(event)
    assert "art_exhibits" in flags, f"Art event should map to art_exhibits even with missing fields, got {flags}"
...
def test_map_event_to_flags_case_insensitive():
    ...
    flags = map_event_to_flags(event)
    assert "art_exhibits" in flags, f"Uppercase art event should still map to art_exhibits, got {flags}"
