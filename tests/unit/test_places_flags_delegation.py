...
class TestPlacesServiceFlagsDelegation:
    ...
    def test_get_places_by_category_delegates_to_shared_mapper(
        self, mock_categories_to_flags
    ):
        ...
        with patch.object(service, "_get_cached_places") as mock_get_cached:
            ...
            with patch(
                "packages.wp_places.service.PlacesService.get_places_by_flags"
            ) as mock_db_get:
    ...
    def test_place_facets_imports_shared_mapper(self):
        ...
        assert (
            "from packages.wp_tags.facets import" in content
        ), "place_facets.py should import from shared facets.py"
        assert (
            "map_event_to_flags" in content
        ), "Should import map_event_to_flags from shared mapper"
        assert (
            "categories_to_facets" in content
        ), "Should import categories_to_facets from shared mapper"

    def test_fallback_implementation_exists(self):
        """Test that fallback implementations exist for robustness."""
        from packages.wp_tags.place_facets import (
            _fallback_map_place_to_flags,
            _fallback_categories_to_place_flags,
        )
    ...
    def test_shared_facets_has_canonical_flags(self):
        ...
        for flag in expected_flags:
            assert (
                flag in content
            ), f"Canonical flag '{flag}' not found in shared mapper.py"
