...
class TestQualityEngine(unittest.TestCase):
    ...
    def test_photo_requirement_disabled(self):
        ...
        engine_no_photo = create_quality_engine(
            min_completeness=0.7, require_photo=False
        )

        # Test place without photos (should be accepted if require_photo=False)
        place_data = self.test_places[5]  # No photos
        is_acceptable, metrics, details = engine_no_photo.assess_place_quality(
            place_data
        )
    ...
    def test_recommendations_and_warnings(self):
        ...
        if metrics.completeness < self.engine.min_completeness:
            self.assertTrue(
                any("Completeness below threshold" in warning for warning in warnings)
            )
    ...
    def test_url_validation(self):
        ...
        # Test invalid URLs
        invalid_urls = ["not-a-url", "ftp://example.com", "example.com", "https://", ""]
    ...
    def test_edge_cases(self):
        ...
        none_place = {"name": None, "city": None, "domain": None, "url": None}
