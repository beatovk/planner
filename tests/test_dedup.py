...
class TestDedupEngine(unittest.TestCase):
    ...
    def test_address_normalization(self):
        ...
        for input_address, expected_normalized in test_cases:
            ...
            if input_address:
                self.assertEqual(candidate.address_hash, candidate.address_hash)  # Should be consistent
            else:
    ...
    def test_identity_key_deduplication(self):
        ...
        is_duplicate, duplicate_id = self.engine.add_place(self.test_places[0])  # place_1
        self.assertFalse(is_duplicate)
        
        is_duplicate, duplicate_id = self.engine.add_place(self.test_places[1])  # place_2 (same core data)
        self.assertTrue(is_duplicate)
        self.assertEqual(duplicate_id, 'place_1')
    ...
    def test_fuzzy_name_deduplication(self):
        ...
        is_duplicate, duplicate_id = self.engine.add_place(self.test_places[6])  # place_7 (typo: "Restaurnt")
        self.assertTrue(is_duplicate)
        self.assertEqual(duplicate_id, 'place_1')
    ...
    def test_address_deduplication(self):
        ...
        is_duplicate, duplicate_id = self.engine.add_place(self.test_places[7])  # place_8 (same address, different name)
        self.assertTrue(is_duplicate)
        self.assertEqual(duplicate_id, 'place_1')
    ...
    def test_geo_deduplication(self):
        ...
        is_duplicate, duplicate_id = self.engine.add_place(self.test_places[8])  # place_9 (same geo, different address format)
        self.assertTrue(is_duplicate)
        self.assertEqual(duplicate_id, 'place_1')
    ...
    def test_duplicate_groups(self):
        ...
        for group in duplicate_groups:
            ...
            for other_place_id in group[1:]:
                ...
                self.assertTrue(
                    (first_place.name_normalized == other_place.name_normalized) or
                    (first_place.address_hash == other_place.address_hash) or
                    (first_place.identity_key == other_place.identity_key)
                )
    ...
    def test_different_cities_not_duplicates(self):
        ...
        is_duplicate, duplicate_id = self.engine.add_place(self.test_places[0])  # Bangkok
        self.assertFalse(is_duplicate)
        
        # Add same name in Chiang Mai (should not be duplicate)
        is_duplicate, duplicate_id = self.engine.add_place(self.test_places[4])  # Chiang Mai
        self.assertFalse(is_duplicate)
    ...
    def test_different_domains_not_duplicates(self):
        ...
        is_duplicate, duplicate_id = self.engine.add_place(self.test_places[0])  # timeout.com
        self.assertFalse(is_duplicate)
        
        # Add same place from bk-magazine.com (should not be duplicate by default)
        is_duplicate, duplicate_id = self.engine.add_place(self.test_places[5])  # bk-magazine.com
        self.assertFalse(is_duplicate)
    ...
    def test_fuzzy_threshold_adjustment(self):
        ...
        # Add place
        is_duplicate, duplicate_id = strict_engine.add_place(self.test_places[0])  # place_1
        self.assertFalse(is_duplicate)
        
        # Add place with typo (should not be duplicate with higher threshold)
        is_duplicate, duplicate_id = strict_engine.add_place(self.test_places[6])  # place_7 (typo)
    ...
    def test_geo_tolerance_adjustment(self):
        ...
        # Add place
        is_duplicate, duplicate_id = strict_geo_engine.add_place(self.test_places[0])  # place_1
        self.assertFalse(is_duplicate)
        
        # Add place with slightly different coordinates (should not be duplicate with strict tolerance)
        is_duplicate, duplicate_id = strict_geo_engine.add_place(self.test_places[8])  # place_9 (same geo)
        self.assertFalse(is_duplicate)  # Should not be considered duplicate due to strict geo tolerance
