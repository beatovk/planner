...
def test_events_cache_enabled():
    ...
    with patch('core.cache.ensure_client', return_value=fake_redis), \
        ...
        assert debug1["cache"]["status"] in ["MISS", "db"], f"Expected MISS or db, got {debug1['cache']['status']}"
        ...
        assert debug2["cache"]["status"] in ["HIT", "STALE"], f"Expected HIT or STALE, got {debug2['cache']['status']}"
        assert debug2["source"] == "redis", f"Expected source=redis, got {debug2['source']}"
        assert debug2["cache"]["keys_checked"], "keys_checked must not be empty on second call"
        
        # Проверяем что события читаются из кэша
        events2 = res2.json()["events"]
        assert len(events2) > 0, "Should have events from cache"
        
        print(f"First call: {debug1['cache']['status']}, Second call: {debug2['cache']['status']}")
