...
class TestArtCacheDBFlow:
    ...
    def test_art_events_in_database(self):
        ...
        for event in events:
            ...
            assert (
                hasattr(event, "source") and event.source == "test"
            ), "Event should have test source"
            assert (
                hasattr(event, "tags") and "art" in event.tags
            ), "Event should have art tag"
    ...
    def test_cache_write_and_read(self):
        ...
        try:
            ...
            assert len(cached_event_ids) == len(
                events
            ), "Should have same number of event IDs"
    ...
    def test_cache_performance(self):
        ...
        assert (
            cache_time < db_time
        ), f"Cache should be faster than DB: cache={cache_time:.3f}s, db={db_time:.3f}s"
