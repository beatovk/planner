...
class BKMagazinePlacesFetcher(FetcherPlaceInterface):
    """BK Magazine: places discovery → detail parsing → normalized Place."""
    name = "bk_magazine_places"
    SOURCE = "bk_magazine"
    CITY = "bangkok"
    
    # URL для мест в BK Magazine
    _LISTING_URLS = [
        "https://bk.asia-city.com/bangkok/restaurants",
        "https://bk.asia-city.com/bangkok/bars",
        "https://bk.asia-city.com/bangkok/arts-culture",
        "https://bk.asia-city.com/bangkok/shopping",
        "https://bk.asia-city.com/bangkok/wellness",
        "https://bk.asia-city.com/bangkok/attractions",
    ]
    
    _CONCURRENCY = int(os.environ.get("BK_CONCURRENCY", "4"))
    _TIMEOUT = float(os.environ.get("BK_TIMEOUT_S", "10"))
    _UA = os.environ.get("BK_UA", "Mozilla/5.0 (WeekPlanner/BKMagazinePlacesFetcher)")

    # Селекторы для мест в BK Magazine
    SELECTORS: Dict[str, str] = {
        "card": ".article, .post, .listing",
        "title": "h2, h3, .title, .headline",
        "url": "a[href]",
        "img": "img",
        "summary": ".excerpt, .summary, .description",
        "tags": ".tags a, .category, .cuisine",
    ...
    def fetch(self, city: str, category: Optional[str] = None, limit: Optional[int] = None) -> List[Place]:
        ...
        try:
            ...
            logger.warning("bk_magazine_places fetch failed: %s", exc)
            return []
        return raw
    ...
    def _extract_listing_links(self, html: str) -> List[str]:
        ...
        # Ищем ссылки на места в BK Magazine
        selectors = [
            "a[href*='/bangkok/']",
            ".article a[href]",
            ".post a[href]",
        ...
        for selector in selectors:
            for a in soup.select(selector):
                ...
                if href and href.startswith("/"):
                    href = f"https://bk.asia-city.com{href}"
    ...
    def _extract_image(self, soup: BeautifulSoup) -> Optional[str]:
        ...
        if img and img.get("src"):
            ...
            if src.startswith("//"):
                ...
                return f"https://bk.asia-city.com{src}"
